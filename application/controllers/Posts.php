...
class Posts extends Member_Controller
{
    ...
    public function index($category_id = 1) 
    {
        ...
        $this->load->view('posts/list_view', $data);
    }

    public function view($post_id)
    {
        $data['post'] = $this->Post_model->get_post_by_id($post_id);
        $this->load->view('posts/detail_view', $data);
    }
    ...
    public function write_form()
    {
        ...
        // 추후 파일도 업로드 가능하도록 변경
        $data['categories'] = $this->Category_model->get_all_categoreis();
        $data['posts'] = null;

        $this->load->view('posts/writeform_view', $data);
    }
    ...
    public function write_process()
    {
        ...
        $this->form_validation->set_rules('boby', '내용', 'required');
        ...
        redirect('posts/detail_view' . $new_post_id);
    }
    ...
    public function edit_form($post_id)
    {
        ...
        if ($post->user_id != $this->session->userdata('id')) {
            ...
            redirect('posts/detail_view' . $post_id);
        }
